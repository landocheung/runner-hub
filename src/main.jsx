...
function App(){
 ...
 return <div className="app">
   <header><div><div className="eyebrow">EPILEPSY ACTION - AJ Bell Great North Run 2026</div><h1>Runner Hub</h1></div><div className="headerTools"><div className="mode">{supabase?'Live database':'Local demo mode'}</div><label className="logoBox" title="Upload a square event logo">{settings.logo_data_url?<img src={settings.logo_data_url} alt="Event logo"/>:<span>Upload<br/>Logo</span>}<input type="file" accept="image/*" onChange={e=>e.target.files[0]&&uploadLogo(e.target.files[0])}/></label></div></header>
